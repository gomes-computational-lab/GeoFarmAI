...
from .clustering import run_agglomerative, run_fcm, run_gmm, run_kmeans
...
def write_run_manifest(manifest_path: Path, fingerprint: str, result: Dict[str, Any]) -> None:
    cached_result = {key: value for key, value in result.items() if key != "best_labels"}
    manifest = {
...
def run_raster_mzd_flow(
    ...
    if OrdinaryKriging is None:
        raise ImportError(
            "PyKrige is required for the raster-first workflow. "
            "Install GeoFarmAI with its GIS dependencies."
        )
    ...
    visual_manifest_path = write_artifact_manifest(run_dir, result)
    result["visual_manifest"] = str(visual_manifest_path)
...
def build_arcgis_grid(x: np.ndarray, y: np.ndarray, buffer: float = 0.0, cell_size: Optional[float] = None) -> Tuple[np.ndarray, np.ndarray, float]:
...
def spatial_pca_from_stack(stack: np.ndarray, valid_mask: np.ndarray, profile: Dict[str, Any], cfg: Dict[str, Any], variables: List[str]):
    ...
    if cfg["spatial_pca"].get("engine", "multispaeti") == "multispaeti" and MultispatiPCA is None:
        warnings.warn(
            "Python multispaeti is unavailable; preserving the historical standard-PCA fallback. "
            "Install the 'multispati' optional dependency to use MultispatiPCA.",
            RuntimeWarning,
            stacklevel=2,
        )
